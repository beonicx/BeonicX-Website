  ...
  return (
    <div className={darkMode ? 'dark' : ''}>
      ...
      <main className={`min-h-screen pt-24 ${darkMode ? 'bg-slate-950' : 'bg-slate-50'}`}>
                ...
                <div className="relative">
                  <input
                    ...
                    className={`w-full px-6 py-4 pl-14 rounded-2xl text-lg outline-none transition-all duration-200 ${
                      darkMode
                        ? 'bg-slate-800 border-2 border-slate-700 text-white placeholder-slate-400 focus:border-blue-600'
                        : 'bg-white border-2 border-slate-200 text-slate-900 placeholder-slate-400 focus:border-blue-600'
                    }`}
                  />
        ...
        <div className={`sticky top-20 z-40 ${darkMode ? 'bg-slate-950/95' : 'bg-slate-50/95'} backdrop-blur-md border-b ${darkMode ? 'border-slate-800' : 'border-slate-200'}`}>
        ...
        <div className="container mx-auto max-w-6xl px-4 py-12">
          {filteredPosts.length === 0 ? (
            <div className={`text-center py-20 ${darkMode ? 'text-slate-400' : 'text-slate-600'}`}>
                ...
                >
                  <Link href={`/blog/${post.slug}`}>
                    ...
                    <div className="p-6">
                      ...
                      <p className={`text-sm mb-4 line-clamp-3 ${darkMode ? 'text-slate-300' : 'text-slate-600'}`}>
                        {post.excerpt}
                      </p>
